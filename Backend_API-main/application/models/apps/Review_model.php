...
class Review_model extends CI_Model
{
    ...
    {
        $this->db->select("r.*,  CONCAT(u.f_name,' ',u.l_name) as user_name,cr.name as coursename");
    ...
    public function insertvoteReview($userArr, $ishelpful)
    {
        $data = ['voted_users'=> implode(",", $userArr)];
         if ($ishelpful == 1) {
            $this->db->set('helpful_yes_counter', 'helpful_yes_counter+1', FALSE);
        } else {
            $this->db->set('helpful_no_counter', 'helpful_no_counter+1', FALSE);
