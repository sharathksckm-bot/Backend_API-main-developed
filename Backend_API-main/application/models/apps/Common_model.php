...
class Common_model extends CI_Model
{
  ...
  {
     // echo "ttt";exit;
    $this->db->select('sc.id ,sc.name, COUNT(c.sub_category) as totalCount,sc.parent_category as categoryid,cc.level');
  ...
  public function postQuestion($data)
{
  ...
  {
     // echo "ttt";exit;
     // print_r( $ClgRepArr['no_of_review']);exit;
    $this->db->set('no_of_articles_linked', 'no_of_articles_linked + ' . $ClgRepArr['no_of_articles_linked'], FALSE);
    $this->db->set('no_of_brochures_download', 'no_of_brochures_download + ' . $ClgRepArr['no_of_brochures_download'], FALSE);
    $this->db->set('no_of_application_submitted', 'no_of_application_submitted + ' . $ClgRepArr['no_of_application_submitted'], FALSE);
    $this->db->set('no_of_que_asked', 'no_of_que_asked + ' . $ClgRepArr['no_of_que_asked'], FALSE);
  ...
  public function getTrendingSpecilization123($categoryId)
{
    $this->db->select('keyword');
    $this->db->from('courses');
	...
	//-------
	/*public function get_College($categoryId)
	{
		$this->db->select('title,logo,city AS city_name,SUM(college_course.total_fees) AS total_fees_range,rank AS rank');
		$this->db->from('college');
		$this->db->join('category', 'college.categoryid = category.id','left');
		$this->db->join('city','college.cityid = city.id','left');
		$this->db->join('college_course','college_course.collegeid = college.id', 'left');
		$this->db->join('college_ranks','college_ranks.college_id = college.id','left');
		$this->db->where('category.id', $categoryId);
		$this->db->group_by('college.id');
	...
	public function get_College($categoryId)
	{
		$this->db->select('title,logo,city AS city_name,SUM(college_course.total_fees) AS total_fees_range,college_ranks.rank AS rank');
		$this->db->from('college');
		$this->db->join('category', 'college.categoryid = category.id','left');
		$this->db->join('city','college.cityid = city.id','left');
		$this->db->join('college_course','college_course.collegeid = college.id', 'left');
   ...
   /* public function get_Courses()
    ...
    public function countCoursesByCourseId()
    {
        $this->db->select('*');
        $this->db->join('courses', 'courses.id=college_course.courseid', 'left');
        $this->db->join('college', 'college.id=college_course.collegeid', 'left');
        $this->db->like('college.categoryid', $CourseId);
        $this->db->where('college_course.collegeid', $clgID);
        $query =  $this->db->get('college_course')->num_rows();
...
public function getCourseId($course_categoryId){
	
    $this->db->select('*');
    $this->db->from('courses');
    $this->db->where('course_category',$course_categoryId); // added semicolon
    $query = $this->db->get();
    return $query->result_array();
}
	
	public function getQue($getCourseId){
		
		//print_r($getCourseId);exit;
		
	$this->db->select('*');
    $this->db->from('question');
    //$this->db->where('course_id',$getCourseId); // added semicolon
    $query = $this->db->get();
	//echo $this->db->last_query();
    return $query->result_array();
	...
	public function getEngClgSpecialization($courseCatId){
		
		//print_r($courseCatId);exit;
		
	$this->db->select('name,id');
    $this->db->from('courses');
    $this->db->where('course_category',$courseCatId); // added semicolon
    $query = $this->db->get();
	//echo $this->db->last_query();
    return $query->result_array();
	...
	public function getBranchWiseCollege($branchId){
		
		//print_r($courseCatId);exit;
		
	$this->db->select('name,id');
    $this->db->from('college');
    $this->db->where('course_category',$branchId); // added semicolon
    $query = $this->db->get();
	//echo $this->db->last_query();
    return $query->result_array();
  ...
  public function getCareerList($categoryId)
  {
    $this->db->select('c.id,c.title,c.categoryid,c.slug,ct.catname as categoryName,g.image');
    $this->db->from('careers c');

    $this->db->join('gallery g', 'g.postid=c.id', 'left');
    $this->db->join('category ct', 'ct.id=c.categoryid', 'left');
  ...
  {
    $this->db->select('c.*,ct.catname as categoryName,g.image');
    $this->db->from('careers c');
  ...
  public function getPlacementDataOfClg($searchCategory,$searchYear,$collegeId)
	...
	public function getTrendingSpecilizationBySubCatId($subcat_Id, $statename)
{
    ini_set('memory_limit', '256M');

    $state_name = $statename[0];

    $this->db->select('c.*, s.statename'); 
    $this->db->from('courses c');
    $this->db->join('college co', 'c.course_category = co.categoryid', 'LEFT');
    ...
    $this->db->where('c.sub_category', $subcat_Id);
    $this->db->limit(100); 
